...
namespace HeaLEOO.Controllers
{
    public class DoctorsController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            return View(new { Doctors = doctors, Clinics = clinics });
        }
        ...
        public async Task<IActionResult> Details(int id)
        {
            ...
            var clinics = await _clinicsService.GetAllAsync() ?? new List<Clinics>();
            return View(new { Doctor = doctor, Clinics = clinics });
        }
        ...
        {
            var clinics = await _clinicsService.GetAllAsync() ?? new List<Clinics>();
            return View(new { Doctor = new DoctorsDto(), Clinics = clinics });
        }
        ...
        public async Task<IActionResult> Create(DoctorsDto doctorDto, List<Clinics> clinics)
        {
            ...
            {
                return View(new { Doctor = doctorDto, Clinics = clinics ?? await _clinicsService.GetAllAsync() ?? new List<Clinics>() });
            }
            ...
            if (doctorDto.ClinicIds != null && doctorDto.ClinicIds.Any())
            {
                foreach (var clinicId in doctorDto.ClinicIds)
                {
                    if (!availableClinics.Any(c => c.Id == clinicId))
                    {
                        ...
                        return View(new { Doctor = doctorDto, Clinics = clinics ?? availableClinics });
                    }
                }
            }
